...
class LixilShutterApiClientError(Exception):
    """Base exception to indicate a general API error."""


class LixilShutterApiClientCommunicationError(
    LixilShutterApiClientError,
):
    """Exception to indicate a communication error with the API."""


class LixilShutterApiClientAuthenticationError(
    LixilShutterApiClientError,
):
...
def _verify_response_or_raise(response: aiohttp.ClientResponse) -> None:
    ...
    Raises:
        LixilShutterApiClientAuthenticationError: For 401/403 errors.
        aiohttp.ClientResponseError: For other HTTP errors.

    """
    if response.status in (401, 403):
        msg = "Invalid credentials"
        raise LixilShutterApiClientAuthenticationError(
            msg,
        )
    response.raise_for_status()


class LixilShutterApiClient:
    """
    ...
    async def async_get_data(self) -> Any:
        ...
        Raises:
            LixilShutterApiClientAuthenticationError: If authentication fails.
            LixilShutterApiClientCommunicationError: If communication fails.
            LixilShutterApiClientError: For other API errors.
    ...
    async def _api_wrapper(
        ...
        try:
            ...
            raise LixilShutterApiClientCommunicationError(
                msg,
            ) from exception
            ...
            raise LixilShutterApiClientError(
                msg,
            ) from exception
