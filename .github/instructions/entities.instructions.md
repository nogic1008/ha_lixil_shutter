...
- **`entity/`** - Base entity classes (inherit `LixilShutterEntity` from `entity/base.py`)
...
**MUST inherit from:** `(PlatformEntity, LixilShutterEntity)` - order matters for MRO
...
**Imports pattern:** `from homeassistant.components.PLATFORM import PlatformEntity, PlatformEntityDescription` + `from ..entity import LixilShutterEntity`
