...
**Directory:** `custom_components/lixil_shutter/[platform]/`
...
```python
"""[Platform] platform for Lixil Bluetooth Shutter."""
...
from .entity import LixilShutterEntity
from .[entity_file] import LixilShutter[EntityName]
...
async def async_setup_entry(
    ...
    coordinator: LixilShutterDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            LixilShutter[EntityName](coordinator, entry),
...
```python
"""[Entity description] for Lixil Bluetooth Shutter."""
...
Add platform to `custom_components/lixil_shutter/manifest.json`:
...
- [ ] Entity class inherits from both `LixilShutterEntity` and platform class
...
- **Class prefix:** `LixilShutter`
- **Base entity:** `LixilShutterEntity` in `entity/base.py`
- **Coordinator:** `LixilShutterDataUpdateCoordinator`
